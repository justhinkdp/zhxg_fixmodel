...
def key_cv(sentence):
    ...
    for i in range(10,13):
        for line in open(keyword_path +str(i)+".csv",encoding='UTF-8'):
            for w in line.split():
                word2id_cat[w.strip()]=i-10
                # word2id_cat_m[w.strip().decode("utf-8")]=ct
                word2id_cat_m[w.strip()]=i-10
    ...
    # rr=['宽带故障]', '[宽带网速慢]', '[移动网络信号差]', '[其他]']
    rr = ['[WTWJ-KDGZ]', '[WTWJ-KDWSM]', '[WTWJ-YDXHC]', '[others]']
    ct = 0
    ct2 = 0
    oth = 0
    cor = 0
