...
def key_cv(sentence):
    ...
    for i in range(13,16):
        for line in open(keyword_path +str(i)+".csv",encoding='UTF-8'):
            for w in line.split():
                word2id_cat[w.strip()]=i-13
                # word2id_cat_m[w.strip()]=ct
                word2id_cat_m[w.strip()]=i-13
    ...
    # rr=['附属产品加包]', '[单宽新装]', '[单TV新装]', '[其他]']
    rr = ['[JZ-FSCPJB]', '[XZ-DKXZ]', '[XZ-DTVXZ]', '[others]']
    ct = 0
    ct2 = 0
    oth = 0
    cor = 0
